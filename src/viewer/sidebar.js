...
export class Sidebar {
        ...
        elToolbar.append(
            this.createToolIcon(
                ...
                () => {
                    $('#menu_measurements').next().slideDown();
                    let annotation =
                        this.viewer.annotationTool.startInsertion();
    ...
    initScene() {
...

        {
            ...
            elExport.append(`
				...
				<a href="#" download="project.json5"><img name="potree_export_button" src="${potreeIcon}" class="button-icon" style="height: 24px" /></a>
			`);
            ...
            let elDownloadDXF = elExport
                .find('img[name=dxf_export_button]')
                .parent();
        ...
        let meshID = tree.jstree(
            'create_node',
            '#',
            { text: '<b>Meshes</b>', id: 'meshes' },
            'last',
            false,
            false,
        );
        ...
        tree.on('dblclick', '.jstree-anchor', (e) => {
            ...
            if (object instanceof PointCloudTree) {
                ...
                let distance =
                    object.distance > 0 ? object.distance / 4 : 5 * 1000;
        ...
        const onGeopackageAdded = (e) => {
            ...
            for (const layer of geopackage.node.children) {
                ...
                tree.jstree(
                    layer.visible ? 'check_node' : 'uncheck_node',
                    shpPointsID,
                );
            }
        };
    ...
    initClippingTool() {
...

        {
            ...
            elClipMethod
                .find(`input[value=${currentClipMethod}]`)
                .trigger('click');
        }
    ...
    initReturnFilters() {
...

        {
            ...
            sldReturnNumber.slider({
                ...
                values: [0, 7],
                slide: (event, ui) => {
                    this.viewer.setFilterReturnNumberRange(
                        ui.values[0],
                        ui.values[1],
                    );
                },
        ...
        {
            // NUMBER OF RETURNS
            let sldNumberOfReturns = elReturnFilterPanel.find(
                '#sldNumberOfReturns',
            );
            let lblNumberOfReturns = elReturnFilterPanel.find(
                '#lblNumberOfReturns',
            );
            ...
            sldNumberOfReturns.slider({
                ...
                values: [0, 7],
                slide: (event, ui) => {
                    this.viewer.setFilterNumberOfReturnsRange(
                        ui.values[0],
                        ui.values[1],
                    );
                },
...

        {
            let slider = new HierarchicalSlider({
                ...
                slide: (event) => {
                    ...
                    this.viewer.setFilterPointSourceIDRange(
                        values[0],
                        values[1],
                    );
                },
    ...
    initClassificationList() {
        ...
        let addClassificationItem = (code, name) => {
            ...
            elInput.click((event) => {
                this.viewer.setClassificationVisibility(
                    code,
                    event.target.checked,
                );
            });

            let defaultColor = classification.color
                .map((c) => c * 255)
                .join(', ');
        ...
        this.viewer.addEventListener(
            'classification_visibility_changed',
            () => {
                {
                    // set checked state of classification buttons
                    for (const classID of Object.keys(
                        this.viewer.classifications,
                    )) {
    ...
    initAppearance() {
        ...
        this.viewer.addEventListener('edl_radius_changed', (event) => {
            $('#lblEDLRadius')[0].innerHTML = this.viewer
                .getEDLRadius()
                .toFixed(1);
        ...
        this.viewer.addEventListener('background_changed', (event) => {
            $(
                "input[name=background][value='" +
                    this.viewer.getBackground() +
                    "']",
            ).prop('checked', true);
        });
...

        {
            ...
            $(
                `input[name=background_options][value=${currentBackground}]`,
            ).trigger('click');
        }
    ...
    initNavigation() {
        ...
        elNavigation.append(
            this.createToolIcon(
                ...
                () => {
                    const animation = CameraAnimation.defaultFromView(
                        this.viewer,
                    );
    ...
    initSettings() {
        {
            ...
            this.viewer.addEventListener('minnodesize_changed', (event) => {
                $('#lblMinNodeSize').html(
                    parseInt(this.viewer.getMinNodeSize()),
                );
                $('#sldMinNodeSize').slider({
                    value: this.viewer.getMinNodeSize(),
                });
...

        {
            ...
            elSplatQuality
                .find(`input[value=${currentQuality}]`)
                .trigger('click');
        }

        $('#show_bounding_box').click(() => {
            this.viewer.setShowBoundingBox(
                $('#show_bounding_box').prop('checked'),
            );
        });
