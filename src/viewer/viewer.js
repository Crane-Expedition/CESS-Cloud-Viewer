...
export class Viewer extends EventDispatcher {
    constructor(domElement, args = {}) {
        ...
        try {
            ...
            this.pointCloudLoadedCallback =
                args.onPointCloudLoaded || function () {};
...

            {
                ...
                this.shadowTestCam = new THREE.PerspectiveCamera(
                    90,
                    1,
                    near,
                    far,
            ...
            if (!this.onAnnotationAdded) {
                this.onAnnotationAdded = (e) => {
                    ...
                    e.annotation.traverse((node) => {
                        $('#potree_annotation_container').append(
                            node.domElement,
                        );
    ...
    setBackground(bg) {
        ...
        if (bg === 'skybox') {
            this.skybox = Utils.loadSkybox(
                new URL(Potree.resourcePath + '/textures/skybox/').href,
            );
        }
    ...
    zoomTo(node, factor, animationDuration = 0) {
...

        {
            ...
            let tween = new TWEEN.Tween(target).to(
                endTarget,
                animationDuration,
            );
            tween.easing(easing);
    ...
    loadSettingsFromURL() {
        if (Utils.getParameterByName('pointSize')) {
            this.setPointSize(
                parseFloat(Utils.getParameterByName('pointSize')),
            );
        }
        ...
        if (Utils.getParameterByName('edlRadius')) {
            this.setEDLRadius(
                parseFloat(Utils.getParameterByName('edlRadius')),
            );
        }

        if (Utils.getParameterByName('edlStrength')) {
            this.setEDLStrength(
                parseFloat(Utils.getParameterByName('edlStrength')),
            );
        }

        if (Utils.getParameterByName('pointBudget')) {
            this.setPointBudget(
                parseFloat(Utils.getParameterByName('pointBudget')),
            );
        }

        if (Utils.getParameterByName('showBoundingBox')) {
            let enabled =
                Utils.getParameterByName('showBoundingBox') === 'true';
            if (enabled) {
                ...
                i18n.init(
                    {
                        lng: 'en',
                        resGetPath:
                            Potree.resourcePath + '/lang/__lng__/__ns__.json',
            ...
            for (const item of event.dataTransfer.items) {
                ...
                if (isJson) {
                    try {
                        ...
                        console.error(
                            'failed to parse the dropped file as JSON',
                        );
                        console.error(e);
                    }
                    ...
                    if (!hasPointcloud) {
                        let msg =
                            'At least one point cloud is needed that specifies the ';
                        msg +=
                            'coordinate reference system before loading vector data.';
                        console.error(msg);
                    } else {
                        proj4.defs(
                            'WGS84',
    ...
    initThree() {
        ...
        if (gl.createVertexArray == null) {
            ...
            if (!extVAO) {
                throw new Error(
                    'OES_vertex_array_object extension not supported',
                );
            }
        ...
        this.scene.annotations.traverse((annotation) => {
...

            {
                ...
                if (viewer.scene.cameraMode == CameraMode.PERSPECTIVE) {
                    ...
                    let projFactor =
                        (0.5 * renderAreaSize.y) / (slope * distance);
            ...
            let zIndex =
                10000000 - distance * (10000000 / this.scene.cameraP.far);
    ...
    update(delta, timestamp) {
        ...
        if (!this.freeze) {
            ...
            if (result.lowestSpacing !== Infinity) {
                ...
                let far = -this.getBoundingBox().applyMatrix4(
        ...
        if (controls === this.deviceControls) {
            ...
            if (typeof debugDisabled === 'undefined') {
                ...
                this.scene.cameraP.rotation.x =
                    Math.PI / 2 + this.scene.view.pitch;
...

        {
            ...
            if (!this._previousCamera.matrixWorld.equals(camera.matrixWorld)) {
                ...
                !this._previousCamera.projectionMatrix.equals(
    ...
    resolveTimings(timestamp) {
        if (Potree.measureTimings) {
            ...
            if (duration > 1000.0) {
                ...
                let glQueries = Potree.resolveQueries(
                ...
                for (let [name, group] of groups) {
                    ...
                    if (group.n === 1) {
                        ...
                        group.median =
                            group.measures[parseInt(group.n / 2)].duration;
                    }
                }

                let cn =
                    Array.from(names).reduce(
                        (a, i) => Math.max(a, i.length),
                        0,
                    ) + 5;
                let cmin = 10;
                let cmed = 10;
